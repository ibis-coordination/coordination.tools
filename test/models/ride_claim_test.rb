...
class RideClaimTest < ActiveSupport::TestCase
  test "pickup location is optional but the seat count must be sane" do
    ...
    assert ride.ride_claims.new(carpool: carpool, user: rider).valid?
    assert_not ride.ride_claims.new(carpool: carpool, user: rider, seats: 0).valid?
